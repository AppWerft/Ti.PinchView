...
#define TAG "ti.pinchview"

using namespace v8;

static Persistent<Object> bindingCache;

static void Pinch_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Pinch.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::PinchBindings::lookupGeneratedInit(
...
static void Pinch_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Pinch_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Pinch_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::PinchBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PinchBinding = {
	"ti.pinchview",
	Pinch_init,
	Pinch_dispose
};
...
Java_ti_pinchview_PinchBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.pinchview", &PinchBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::PinchBindings::lookupGeneratedInit));
}
